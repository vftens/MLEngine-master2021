...
namespace MLEngine.Models
{
    public class Listener
    {
        ...
        List<TypedMessageBus> tinyMessageBuses = new List<TypedMessageBus>();
        ...
        public Listener (ObservableCollection<MLmodel> mlModels)
        {
            ...
            try
            {
                foreach (var model in MLmodels)
                {
                    ...
                    tinyMessageBuses.Add(new TypedMessageBus(model.PostfixModel));
                }
            }
        ...
        public void Stop()
        {
            ...
            {
                m_Cts.Cancel();
                foreach (var bus in tinyMessageBuses)
                    bus.Stop();
            }
        }
        ...
        private void Listen(CancellationToken token)
        {
            ...
            {
                bus.Start();
                bus.NewMessageRecived += Bus_NewMessageRecived;
            }
        }

        private async void Bus_NewMessageRecived(string channelName, string message)
        {
